...
const StockData = () => {
  ...
  const API_KEY = "sk_30fd4380631243f3a6858b896703722f"; // Replace with your actual API key

  useEffect(() => {
    const fetchOptionsSymbols = async () => {
      try {
        const optionsResponse = await fetch(
          `https://cloud.iexapis.com/stable/ref-data/options/symbols?token=${API_KEY}`
        );
        if (optionsResponse.ok) {
          const optionsData = await optionsResponse.json();
          const symbolsArray = Object.entries(optionsData).reduce(
            (acc, [symbol, nums]) => {
    ...
    const fetchStockData = async (symbols) => {
      try {
        const response = await fetch(
          `https://cloud.iexapis.com/stable/stock/market/batch?symbols=${symbols.join(
            ","
          )}&types=quote&token=${API_KEY}`
        );
        if (response.ok) {
          ...
          {Object.keys(stockData).length > 0 ? (
            Object.keys(stockData).map((symbol) => (
              ...
              >
                ...
                {stockData[symbol].quote &&
                  stockData[symbol].quote.currency && (
                    <p>Currency: {stockData[symbol].quote.currency}</p>
                  )}
                {stockData[symbol].quote &&
                  stockData[symbol].quote.latestPrice && (
